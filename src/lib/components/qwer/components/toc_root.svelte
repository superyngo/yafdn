<script lang="ts">
  import type {TOC} from "$lib/components/qwer/types/toc";
  import TocContent from "$lib/components/qwer/components/toc_content.svelte";
  ...
  $: if (browser) {
    ...
    const lastActivated =
      activated.length > 0
        ? document.getElementById(activated[activated.length - 1])
        : undefined;
...
{#if toc && toc.length > 0}
  <aside
    ...
    class="sticky top-[4rem] hidden xl:block pb8"
  >
    <div
      role="button"
      tabindex="0"
    ...
    >
      ...
      >
        <div
          class="i-bxs-chevrons-up w6 h6 m-auto {upMore ? 'op100' : 'op0'}"
        />
      </div>
      {#if toc && toc.length > 0}
        <ul
      ...
      >
        <div
          class="i-bxs-chevrons-down w6 h6 m-auto {downMore ? 'op100 ' : 'op0'}"
        />
      </div>
    </div>
  </aside>
