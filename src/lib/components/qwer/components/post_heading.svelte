<script lang="ts">
  ...
  import {
    ...
    defaultPublishedStr,
    defaultUpdatedStr,
  } from "$lib/components/qwer/utli/timeFormat";
...
<div class="flex flex-col pt8 mx8">
  <div class="flex justify-between items-center mx--4 md:mx0">
    <a
      class="hidden u-url u-uid"
      href={new URL(data.slug, siteConfig.url).href}
    >
    ...
    <div class="flex flex-col gap1 text-right text-sm font-semibold op80">
      <time
        use:tippy
        class="dt-published"
        aria-label="{$LL.FirstPublishedAt()} {new Date(
          data.published
        ).toLocaleString(dateConfig.toPublishedString.locales, {
          year: 'numeric',
          month: 'numeric',
          day: 'numeric',
          hour: 'numeric',
      ...
      <span
        use:tippy
        aria-label="{$LL.LastUpdatedAt()} {new Date(
          data.updated
        ).toLocaleString(dateConfig.toPublishedString.locales, {
          year: 'numeric',
          month: 'numeric',
  ...
  <h1 itemprop="name headline" class="p-name text-4xl my4 mx--4 md:mx0">
    {data.title}
  </h1>
